...
def sample_project(tmp_path):
    ...
    packages = {"packages": [{"package": "datavault-uk/automate_dv", "version": "0.10.0"}]}
    ...
    metadv_config = {
        "metadv": {
            "targets": [
                {"name": "customer", "type": "entity", "description": "Customer entity"},
                {"name": "product", "type": "entity", "description": "Product entity"},
            ],
            "sources": [
                {
                    "name": "stg_customers",
                    "columns": [
                        ...
                        {"name": "customer_name", "target": [{"attribute_of": "customer"}]},
                    ],
                },
                {
                    "name": "stg_products",
                    "columns": [
                        {"name": "product_id", "target": [{"target_name": "product"}]},
                        {"name": "product_name", "target": [{"attribute_of": "product"}]},
                    ],
                },
            ],
        }
...
class TestMetaDVGenerator:
    ...
    def test_generator_with_datavault4dbt(self, sample_project):
        ...
        packages = {"packages": [{"package": "scalefreecom/datavault4dbt", "version": "1.0.0"}]}
...
class TestMultiactiveSatellite:
    ...
    def test_multiactive_satellite(self, tmp_path):
        ...
        metadv_config = {
            "metadv": {
                "targets": [{"name": "customer", "type": "entity"}],
                "sources": [
                    {
                        "name": "stg_phones",
                        "columns": [
                            ...
                            {
                                "name": "phone_type",
                                "target": [{"attribute_of": "customer", "multiactive_key": True}],
                            },
...
class TestValidation:
    ...
    def test_validation_entity_no_source(self, tmp_path):
        ...
        metadv_config = {
            "metadv": {"targets": [{"name": "customer", "type": "entity"}], "sources": []}
        }
        with open(metadv_dir / "metadv.yml", "w") as f:
            yaml.dump(metadv_config, f)
